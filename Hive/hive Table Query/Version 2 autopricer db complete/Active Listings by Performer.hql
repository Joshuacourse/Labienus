...
select 
                  ...
                  where TO_DATE(eventdatelocal)>=TO_DATE(process_date) 
                  ) t
                  GROUP BY
                  eventdate,
                  process_date,
...
from 
sandbox_dev_bi.josh_clientHealthSummaryByEventIdLatestDate 
...
DROP TABLE sandbox_dev_bi.matt_logs_join_client_join_activeListing_added_eventid PURGE;

CREATE TABLE IF NOT EXISTS sandbox_dev_bi.matt_logs_join_client_join_activeListing_added_eventid
(                         
primary_performer         STRING,
venue_name                STRING,
company_name              STRING,
...
set mapred.job.queue.name=root.dev;
insert OVERWRITE TABLE sandbox_dev_bi.matt_logs_join_client_join_activeListing_added_eventid
SELECT
...
AND t1.user_id = t2.client_id
group by 
t1.primary_performer, t1.venue_name,t1.category, t2.company_name,t2.acc_type, t1.event_id;
