CREATE TABLE IF NOT EXISTS sandbox_dev_bi.daily_sales_aggregate_overwrite
(
    unique_id                                  STRING,
    user_api                                   STRING,
    event_id                                   STRING,
...
CREATE TABLE IF NOT EXISTS sandbox_dev_bi.daily_previous_day_sale_aggregate_overwrite
(
    unique_id                                  STRING,
    user_api                                   STRING,
    event_id                                   STRING,
...
CREATE TABLE IF NOT EXISTS sandbox_dev_bi.daily_past_week_sale_aggregate_overwrite
(
    unique_id                                  STRING,
    user_api                                   STRING,
    event_id                                   STRING,
...
CREATE TABLE IF NOT EXISTS sandbox_dev_bi.sales_data_daily_aggregate
(
    unique_id                                 STRING,
    user_api                                  STRING,
    event_id                                  STRING,
...
INSERT OVERWRITE TABLE sandbox_dev_bi.daily_sales_aggregate_overwrite 
SELECT 
...
FROM prod_etl_dq.user_sales_standardized_all AS a 
JOIN 
...
INSERT OVERWRITE TABLE sandbox_dev_bi.daily_past_week_sale_aggregate_overwrite
SELECT
reflect('org.apache.commons.codec.digest.DigestUtils', 'md5Hex', concat(a.user_api,a.bg_eventid)) as unique_id,
a.user_api                                          AS user_api,
a.bg_eventid                                        AS event_id,
...
INSERT OVERWRITE TABLE sandbox_dev_bi.daily_previous_day_sale_aggregate_overwrite
SELECT
reflect('org.apache.commons.codec.digest.DigestUtils', 'md5Hex', concat(a.user_api,a.bg_eventid)) as unique_id,
a.user_api                                          AS user_api,
a.bg_eventid                                        AS event_id,
...
from sandbox_dev_bi.sales_data_daily_aggregate
...
DROP TABLE sandbox_dev_bi.josh_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply PURGE;
CREATE TABLE IF NOT EXISTS sandbox_dev_bi.josh_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply
(
event_id                              STRING,
user_api                              STRING,
acc_email                             STRING,
...
set mapred.job.queue.name=root.dev;
INSERT OVERWRITE TABLE  sandbox_dev_bi.josh_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply
select 
t1.event_id, t1.user_api, t1.acc_email, t1.transaction_date, t1.Create1, t1.Edit1, t1.Save1,
t1.last_touched_non_FS_pricer, t1.last_touched_FS_pricer, t1.last_touched_date_non_FS, t1.last_touched_date_FS,
...
left join sandbox_dev_bi.josh_StubHubSupply_All AS t2
on t1.event_id = t2.event_id;
...
DROP TABLE sandbox_dev_bi.matt_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply_shrink PURGE;
CREATE TABLE IF NOT EXISTS sandbox_dev_bi.matt_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply_shrink
(
event_id                              STRING,
user_api                              STRING,
acc_email                             STRING,
...
INSERT OVERWRITE TABLE  sandbox_dev_bi.matt_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply_shrink
select  distinct
event_id                              ,
user_api                              ,
acc_email                             ,
...
from 
sandbox_dev_bi.josh_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply
where 
...
DROP TABLE sandbox_dev_bi.matt_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply_shrink_overall PURGE;
CREATE TABLE IF NOT EXISTS sandbox_dev_bi.matt_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply_shrink_overall
(
event_id                              STRING,
user_api                              STRING,
last_touched_non_FS_pricer            STRING,
...
INSERT OVERWRITE TABLE  sandbox_dev_bi.matt_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply_shrink_overall
select  distinct
event_id                              ,
user_api                              ,
last_touched_non_FS_pricer            ,
...
DROP TABLE sandbox_dev_bi.last_process_date_v2 PURGE;


CREATE TABLE IF NOT EXISTS sandbox_dev_bi.last_process_date_v2
(
last_process_date        DATE
);
set mapred.job.queue.name=root.dev;
INSERT OVERWRITE TABLE  sandbox_dev_bi.last_process_date_v2
select max(TO_DATE(process_date)) from autopricer_db.listing_criteria AS last_process_date;
...
DROP TABLE sandbox_dev_bi.clientHealthSummaryByEventId_v2 PURGE;


CREATE TABLE IF NOT EXISTS sandbox_dev_bi.clientHealthSummaryByEventId_v2
(
eventdate                 STRING, 
process_date              STRING, 
category                  STRING,
...
INSERT OVERWRITE TABLE  sandbox_dev_bi.clientHealthSummaryByEventId_v2
select 
                  ...
                  split(criteria,"[//|]")[4]                                             AS floor,
                  split(criteria,"[//|]")[6]                                             AS pack,
                  actual_price                                                           AS actual_price,
                  changed_price                                                          AS changed_price,
                  case 
                  when price_floor = 1     then (split(criteria,"[//|]")[4]/changed_price)
                  else                          0           end                          AS floor_to_cp_ratio
                  
                  from 
                  autopricer_db.listing_criteria  t1
                  join (select * from sandbox_dev_bi.last_process_date_v2) t2
                  on t1.process_date = t2.last_process_date
                  where TO_DATE(t1.eventdatelocal)>=TO_DATE(t1.process_date)
                  ) t
                  GROUP BY
                  eventdate,
                  process_date,
...
DROP TABLE sandbox_dev_bi.clientHealthSummaryByEventId_totalListingsTotalTickets PURGE;


CREATE TABLE IF NOT EXISTS sandbox_dev_bi.clientHealthSummaryByEventId_totalListingsTotalTickets
(
eventdate                 STRING, 
process_date              STRING, 
category                  STRING,
...
INSERT OVERWRITE TABLE  sandbox_dev_bi.clientHealthSummaryByEventId_totalListingsTotalTickets
select 
                  ...
                  from 
                  sandbox_dev_bi.clientHealthSummaryByEventId_v2  t1
                  GROUP BY
                  eventdate,
                  process_date,
                  category,
...
DROP TABLE sandbox_dev_bi.clientHealthSummaryByEventId_v3 PURGE;


CREATE TABLE IF NOT EXISTS sandbox_dev_bi.clientHealthSummaryByEventId_v3
(
eventdate                 STRING, 
process_date              STRING, 
category                  STRING,
...
INSERT OVERWRITE TABLE  sandbox_dev_bi.clientHealthSummaryByEventId_v3
select 
                  t1.eventdate                                                     AS eventdate, 
                  t1.process_date                                                  AS process_date, 
                  t1.category                                                      AS category,
                  ...
                  from 
                  sandbox_dev_bi.clientHealthSummaryByEventId_totalListingsTotalTickets  t1
                  join (
                        select 
                        ...
                        from 
                        sandbox_dev_bi.clientHealthSummaryByEventId_v2 where activeorsold >0 and totallistings>0 
                        ) t2
...
DROP TABLE sandbox_dev_bi.pricer_inventory PURGE;


CREATE TABLE IF NOT EXISTS sandbox_dev_bi.pricer_inventory
(
eventdate                 STRING, 
process_date              STRING, 
category                  STRING,
...
insert OVERWRITE table sandbox_dev_bi.pricer_inventory
select  
...
from 
sandbox_dev_bi.clientHealthSummaryByEventId_v3 a
join 
sandbox_dev_bi.clientIdLookUpTable b
on a.user_id = b.client_id
join
sandbox_dev_bi.matt_reshape_click_initial_ClientSales_StubHubSales_StubHubSupply_shrink c
on b.user_api = c.user_api AND a.event_id = c.event_id
;
